...
App 左下角的 `MQTT: connected/total` 是 MQTT 連線摘要。例如上方設定有兩個 broker，畫面顯示 `MQTT: 2/2` 代表兩個 broker 都已連線；如果是 `MQTT: 0/2` 或紅色，代表語音/動作指令通道尚未連好。
...
### A. TTS 字幕
當後端透過 MQTT 發送 `temi/action/speak` 時，Temi 會播放 TTS，並在畫面底部中央同步顯示小型白色字幕。字幕設計為最多 3 行、半透明黑底，避免遮住太多相機畫面；TTS 完成或錯誤時會自動消失。
...
### B. ASR 等待逾時
ASR 內容送到後端後，App 會進入 `WAITING` 狀態等待後端 action。如果 60 秒內沒有收到 `temi/action/speak`、`temi/action/navigate` 或其他 action，App 端 watchdog 會播放「連線逾時，請稍後再試」並回到 `IDLE`。這是 App 端保護機制，不是 Temi 內建 ASR 功能。

---

## 7. 電腦端接收中樞 (PC Backend / Hermes Agent)
...
*   `AgentStateMachine: Watchdog started for 60000 ms.` ➔ App 已開始等待後端回覆，逾時門檻為 60 秒。
