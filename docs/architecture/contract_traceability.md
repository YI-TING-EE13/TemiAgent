...
| Resident identity result v1.0 | Feature-gated Bridge consumer implemented; producer and real visual acceptance pending | `hermes_temi_bridge/schemas/resident_identity_result.schema.json`, `resident_context.py` | External VLM/identity provider; Temi App manual selection | Bridge consumes only fresh `vision_gender_fallback` father/mother for controlled Demo; Android/report pipeline remain external | schema tests and `test_demo_resident_media_runtime.py`; no provider/Android E2E evidence | Runtime schema, all producers/consumers, Android contract, reader copy and privacy tests |
...
| Care memory formats | Demo-only synthetic data; per-resident private seed is feature-gated | `memory_store.py`, `care_context_builder.py` and `demo_care_memory.py` | Bridge memory actions and explicit local seed tool | Bridge/Hermes context builder; `unknown` reads/writes no private partition | memory/context tests and `test_demo_resident_media_runtime.py` | Code, tests, `memory/README.md`, skill contract and retention rules |
...
- **Contract defined; integration pending:** care report and report interaction result; an upstream identity producer remains pending.
- **Feature-gated integration:** Bridge media v1.1 runtime, resident Hermes native media entry and identity consumer; Android mapping and real-device acceptance pending.
