...
| Care memory formats | Demo-only synthetic data; per-resident private seed is feature-gated | `memory_store.py`, `care_context_builder.py`, `demo_care_memory.py` and `demo_repeated_discomfort.py` | Bridge memory actions, explicit local seed tool and father-only native callback flow | Bridge/Hermes context builder; unknown/mother do not read father partition; repeated flow retrieves one fixed synthetic event then appends only after confirmation/callback success | memory/context tests, `test_demo_resident_media_runtime.py`, `test_demo_identity_repeated_discomfort.py` | Code, tests, `memory/README.md`, skill contract and retention rules |
