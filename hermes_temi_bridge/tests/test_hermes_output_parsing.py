...
from hermes_temi_bridge.hermes_client import HermesOutputError, _build_command, parse_hermes_output
...
class HermesOutputParsingTests(unittest.TestCase):
    ...
    def test_prompt_placeholder_preserves_prompt_as_one_argument(self):
        command = _build_command("hermes -z {prompt}", 'line 1\n{"ok": true}')
        self.assertEqual(command[:2], ["hermes", "-z"])
        self.assertEqual(command[2], 'line 1\n{"ok": true}')

    def test_command_without_placeholder_appends_query_flag(self):
        self.assertEqual(_build_command("hermes chat", "hello"), ["hermes", "chat", "-q", "hello"])
