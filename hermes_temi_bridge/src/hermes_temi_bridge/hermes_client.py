...
def format_care_context_block(care_context: dict[str, Any] | None) -> str:
    ...
    return (
        ...
        "Reminder completion contract:\n"
        "- Emit mark_reminder_done only with the exact non-empty reminder_id from active_reminders; never invent, infer, or omit it.\n"
        "- If exactly one active reminder clearly matches the current utterance, use that exact reminder_id and record the completion.\n"
        "- If no active reminder matches, or more than one reminder could match, ask_clarification and do not emit mark_reminder_done.\n\n"
        "<care_context>\n"
        f"{context_json}\n"
        "</care_context>\n"
    )
